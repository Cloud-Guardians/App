...
const CommentReport = ({diaryId, commentId, visible, accessToken, onClose})=>{
    ...
    const sendReport = async (checkedReason, reasonValue, diaryId, commentId, accessToken)=> {
        console.log("Checked Reason:", checkedReason);
            console.log("Reason Value:", reasonValue);
            console.log("Diary ID:", diaryId);
            console.log("Access Token:", accessToken);
        try {
                                             const response = await fetch(`http://ec2-3-38-253-190.ap-northeast-2.compute.amazonaws.com:9090/api/public-diaries/${diaryId}/comments/${commentId}/reports`, {
                                                 method: 'POST',
                                                 body:JSON.stringify({
   ...
   const toggleCheckBox = (id) => {
    ...
    return(
        ...
        <Images.Cancel style={{margin:"auto"}}/></TouchableOpacity></View>
        </HeaderView>
        <ReportLine/>
        <CheckBoxView>
        {isChecked.map((checked,index) => (
            ...
            />
            <TouchableOpacity style={{width: 200, height: 35, margin:"auto", marginTop:30, backgroundColor:"#35465C", borderRadius:10}} onPress={()=>sendReport(checkedReason, reasonValue, diaryId, commentId, accessToken)}>
